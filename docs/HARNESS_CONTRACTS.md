...
| `mumei_develop_audit` | Develop-checkout target inventory and fixture/live audit evidence with `verification_violations` and `cross_validation_gaps` | `migrate-suggest` skeletons for `scripts/generate_stdlib_metrics.py` | Not used | Summary row with 3/3 proof density |
...
| `blockchain_audit` | Reentrancy/overflow/access-control rejection and `blockchain_audit.proof.json` | Deterministic audit taxonomy preview | `MumeiLean.Blockchain` build and `blockchain_audit.lean-cert.json` when available | Summary row with 5/5 proof density |
